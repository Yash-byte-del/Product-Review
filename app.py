...
if analyze_button:
    if not product_url:
        ...
        c2.metric("Average Rating", f"{avg_rating:.1f}/5.0")
        c3.metric("Authentic Reviews", f"{authentic_count}/{len(df)}")
        c4.metric("Verified Purchases", f"{verified_count}/{len(df)}")
        ...
        with v1:
            ...
            fig_rating = go.Figure(data=[go.Bar(x=rating_counts.index, y=rating_counts.values)])
            fig_rating.update_layout(title="Rating Distribution", xaxis_title="Rating", yaxis_title="Count", height=300)
            st.plotly_chart(fig_rating, use_container_width=True)

        with v2:
            fig_sentiment = go.Figure(data=[go.Histogram(x=df['sentiment_score'], nbinsx=10)])
            fig_sentiment.update_layout(title="Sentiment Score Distribution", xaxis_title="Sentiment Score", yaxis_title="Count", height=300)
        ...
        fig_auth = px.pie(auth_data, values='Count', names='Type', title='Review Authenticity')
        ...
        for idx, row in df.iterrows():
            with st.expander(f"Review {idx+1} - {'*' * row['rating']} | {row['sentiment_label']}"):
                ...
                c1.write(row['text'])
                if row['verified']:
                    c2.markdown("Verified")
                if row['authentic']:
                    c2.markdown("Authentic")
                else:
                    c2.markdown("Suspicious")
                c2.markdown(f"Sentiment: {row['sentiment_score']:.2f}")
...
st.markdown(
    ...
    "Made with Streamlit | © 2024 Amazon Review Classifier"
    "</div>",
    unsafe_allow_html=True
)
